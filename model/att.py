...
class MH_ATT(nn.Module):
    def __init__(self, n_att_heads=4, q_dim=128, k_dim=128, v_dim=128, out_dim=128):
        ...
        self.bond_emb = Embedding(4, out_dim)
        self.edge_proj = nn.Linear(self.d_k*2, 1, bias=True)
    
    def forward(self, query, key, value, query_batch, key_value_batch, bond=None):
        ...
        if bond is not None:
            bond_proj = self.bond_emb(bond).view(-1, self.n_att_heads, self.d_k)
            tmp = torch.cat([query_proj+key_proj, bond_proj], dim=-1)
            scaled_dots += self.edge_proj(tmp).squeeze(dim=-1)
