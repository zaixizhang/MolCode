...
def get_rmsd_confusion_matrix(data: Data, useFF=False):
    ...
    out_path = '/apdcephfs/private_zaixizhang/exp_gen/13/' + 'saved_mol/'+data['smiles']
    ...
    for i in range(num_gen):
        ...
        for j in range(num_ref):
            ...
            if rmsd_confusion_mat[j,i] < best:
                best = rmsd_confusion_mat[j,i]
                Chem.MolToMolFile(gen_mol, out_path+ '/{:}_gen.mol'.format(i))
                Chem.MolToMolFile(ref_mol, out_path+ '/{:}_ref.mol'.format(i))
                Chem.MolToMolFile(ori_mol, out_path + '/{:}_gen_baseline.mol'.format(i))
